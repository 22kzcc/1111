...
function setupUnitThreeModal() {
    ...
    unitThreeContentDiv.child(createP('在此可放置小測驗、互動題目或練習區塊。'));
...
function resetQuiz() {
    quizIndex = 0;
    quizScore = 0;
    // 如果之前清空過 unitThreeContentDiv（顯示結果時），重新準備 UI 結構
    if (!questionEl || !optionButtons || optionButtons.length === 0) {
        // 重新建立 modal 內容（簡單方式：重新呼叫 setupUnitThreeModal）
        unitThreeModal.hide();
        setupUnitThreeModal();
        unitThreeModal.show();
    } else {
        // 恢復 UI 並顯示第一題
        // 若 unitThreeContentDiv 內容被清空，先把標題與按鈕加入回來
        if (!unitThreeContentDiv.elt.childElementCount) {
            unitThreeContentDiv.child(createElement('h1', '作品三：測驗'));
            unitThreeContentDiv.child(createP('在此可放置小測驗、互動題目或練習區塊。'));
            unitThreeContentDiv.child(questionEl);
            optionButtons.forEach(b => unitThreeContentDiv.child(b));
            unitThreeContentDiv.child(feedbackEl);
            unitThreeContentDiv.child(nextQButton);
            unitThreeContentDiv.child(restartButton);
            unitThreeContentDiv.child(exitUnitThreeButton);
        }
        showQuizQuestion();
...
function setupUnitFourModal() {
    ...
    contentDiv.child(createElement('h2', '作品四 — 自我介紹'));
    contentDiv.child(createP('這裡放你的自我介紹、照片或連結。'));
